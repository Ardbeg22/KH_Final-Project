    ...
    <div class="wrap">
        ...
        <div id="content_container">
		    ...
		    <div id="admin_mypage_navi">
		        ...
		        <div id="selected_tab"><a href="reportCommunityManagement.ad">신고 관리</a></div>
			...
			<div id="reportCommunityManagement">
				...
				<div id="reportCategory">
					...
					<div style="text-align:right; color: white; font-size:bold;"><button type="button" onclick="reportedCommunityList();">처리되지 않은 글 보기</button></div>
				</div>
						...
						<c:when test="${ not empty list }">
							<table id="communityTable">
								<thead>
									<tr id="community_head" class="line">
										<th class="community_head1" width="10%;">작성자</th>
								...
								<tbody>
									<c:forEach var="c" items="${ list }">
										<tr class="personalCommunity">
											...
											<td id="community_content">${ c.comTitle }</td>
											...
											<td>
												...
												<button type="button" class="btn btn-danger" id="deleteReportedCommunity">되돌리기</button>
											</td>
										</tr>
									</c:forEach>
								</tbody>
							...
							<div id="pagingArea">
								<c:choose>
									...
									<c:otherwise>
										<button type="button" onclick="location.href='processedCommunityList.ad?cpage=${ pi.currentPage - 1}';">«</button>
									</c:otherwise>
								</c:choose>
								
								<c:forEach var="p" begin="${ pi.startPage }" end="${ pi.endPage }" step="1">
									<button type="button" onclick="location.href='processedCommunityList.ad?cpage=${ p }';">${ p }</button>
								</c:forEach>
				...
				<script>
					$("#communityListAll").on("click", "#deleteReportedCommunity", function(){
						...
						if(confirm("해당 게시글을 되돌리시겠습니까?")) {
							
							$.ajax({
								url : "resetReportedCommunity.ad",
								type : "get",
					...
					function reportedCommunityList() {
